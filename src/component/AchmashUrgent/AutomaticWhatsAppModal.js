...
import automaticWhatsAppSendingService from '../../services/automaticWhatsAppSending.service'
...
const style = {
 ...
 function AutomaticWhatsAppModal(props) {
  ...
  const sendMultiplePhone = () => {
    ...
    let responseObj=automaticWhatsAppSendingService.insertMultipleAutomaticSending(object);
  ...
  useEffect(() => {
    ...
    async function init  () {
      ...
      const whatsAppData=await automaticWhatsAppSendingService.fetchListWhatsAppForAutomaticSend(props.singleMessage.ClientCode_Int);
  ...
  justifyContent="center" >
        <Grid item xs={12}>
            <h2>בחירת נייד למשלוח WhatsApp</h2>
            </Grid>
            <Grid item xs={6}>
